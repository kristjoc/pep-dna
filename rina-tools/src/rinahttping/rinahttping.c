 ...
 *    NWRITE FUNCTION
 */
int Nwrite(int fd, void *buf, size_t len)
{
    size_t nleft = len;
    int rc = 0;
 ...
 *   NREAD FUNCTION
 */
int Nread(int fd, void *buf, size_t len)
{
    size_t  nleft = len;
    int rc = 0;
...
static int prepare_server_socket(void)
{
    ...
    regfd = rina_register(cfd, difName, localAPN, 0);
    if (regfd < 0) {
...
static void server(void)
{
    ...
    int cfd = 0, asock = 0, con = -1;

    cfd = prepare_server_socket();

    while (running) {
        /* wait for a connection request */
        asock = rina_flow_accept(cfd, NULL, NULL, 0);
        if (asock < 0) {
 ...
 *  PARSE GET REQUEST
 */
unsigned int parse_get_request(char *request, char *filename)
{
    struct stat buf;
...
static void webperf()
{
    ...
    while (running) {
        /* wait for a connection request */
        asock[id] = rina_flow_accept(cfd, NULL, NULL, 0);
        if (asock[id] < 0) {
            /* perror("rina_flow_accept"); */
            running = 0;
            break;
        ...
        rc = pthread_create(&thread[id], NULL, pthread_fn,
                (void *)&asock[id]);
        if (rc < 0) {
            perror("pthread_create");
            close(asock[id]);
...
static void cpumem_srv_perf()
{
    ...
    while (running) {
        /* wait for a connection request */
        asock[id] = rina_flow_accept(sock, NULL, NULL, 0);
        if (asock[id] < 0) {
            /* perror("rina_flow_accept"); */
            running = 0;
            break;
