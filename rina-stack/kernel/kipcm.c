...
/* FIXME: This "method" is only temporary, do not rely on its presence */
struct kipcm_pmap * kipcm_messages_ingress(struct kipcm * kipcm)
{
        if (!kipcm) {
                LOG_ERR("Bogus kipcm instance passed, bailing out");
                return NULL;
        ...
        return kipcm->messages->ingress;
}
EXPORT_SYMBOL(kipcm_messages_ingress);
