...
static struct du * red_rmt_dequeue_policy(struct rmt_ps *      ps,
		       			  struct rmt_n1_port * port)
{
	 ...
	 */
	q->vars.qavg = red_calc_qavg(&q->parms, &q->vars, qlen);
				  ...
				  bool                 must_enqueue)
{
	 ...
	 */
	q->vars.qavg = red_calc_qavg(&q->parms, &q->vars, qlen);
	LOG_DBG("qavg': %lu, qlen: %u", q->vars.qavg >> (q->parms.Wlog), qlen);
                                           ...
                                           const char     *value)
{
	...
	if (strcmp(name, "stab_address_p") == 0) {
		...
		int i;
		size_t size = 256;
		u8 * stab_table = rkmalloc(sizeof(*stab_table)*size,
					   GFP_KERNEL);
		ret = kstrtoul(value, 16, (unsigned long *) &user_address);
		if (!ret)
			user_address = 0;
		user_pointer = (void __user *) user_address;
		ret = copy_from_user(stab_table,
				     (const void __user *) user_pointer,
				     size*sizeof(u8));
		if (ret !=0) {
			LOG_ERR("Stab table for RMT's RED PS was not fully "
				"copied, missing %d out of %u bytes",
...
static int rmt_ps_load_param(struct rmt_ps *ps,
...
static struct ps_base * rmt_ps_red_create(struct rina_component * component)
{
